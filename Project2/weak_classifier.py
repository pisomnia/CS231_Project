...
class Ada_Weak_Classifier(Weak_Classifier):
	...
	def calc_error(self, weights, labels):
		######################
		Threshold =np.linspace(min(self.activations),max(self.activations),50)
		Error=np.zeros(Threshold.shape)
		#S=np.array([-1, 1])
		Polarity=np.zeros(Threshold.shape)
		h=np.zeros(self.activations.shape[0])
		ht=np.zeros(self.activations.shape[0])
		for i in range (Threshold.shape[0]):
			ht=(1)*np.sign(self.activations - Threshold[i])
			Error[i]=np.dot(weights,(ht!=labels))
			Polarity[i]=1
			if Error[i] >0.5:
				ht=(-1)*np.sign(self.activations - Threshold[i])
				Error[i]=np.dot(weights,(ht!=labels))
		...
		error=Error[index]
		self.polarity=Polarity[index]
		self.threshold=Threshold[index]
		h=self.polarity*np.sign(self.activations - self.threshold)
		######################
		return error,h
